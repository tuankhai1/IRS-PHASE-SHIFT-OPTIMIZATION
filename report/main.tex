...
\begin{enumerate}
    \item \textbf{Upper bound vs. practical model:} The ideal model (upper bound) consistently outperforms all practical-model schemes, with the gap widening at shorter distances where the IRS contribution is more significant. At $d = 500$ m, the upper bound achieves $4.55$ bits/s/Hz compared to $3.46$ bits/s/Hz for AO practical (Prop. 1), a loss of approximately $24\%$.
    ...
    \item \textbf{Metaheuristic performance:} PSO and GWO closely match the AO practical baseline, achieving within $0.01$--$0.02$ bits/s/Hz across all distances. This validates that population-based metaheuristics can effectively optimize the non-convex phase shift problem.

    \item \textbf{No-IRS lower bound:} Without IRS, the achievable rate remains below $0.17$ bits/s/Hz, demonstrating the dramatic improvement enabled by IRS deployment.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{Scalability of metaheuristics:} PSO and GWO maintain near-optimal performance up to $N = 80$, though a slight performance degradation is observed for PSO at large $N$ (e.g., $3.72$ vs. $3.83$ bits/s/Hz for AO at $N = 80$), as the search space grows linearly with $N$.

    \item \textbf{Practical vs. ideal gap:} The gap between the upper bound and practical-model schemes grows with $N$, from $0.35$ bits/s/Hz at $N = 10$ to $1.18$ bits/s/Hz at $N = 80$, as each additional element introduces amplitude-phase coupling loss.
\end{enumerate}
...
\begin{figure}[htbp]
    ...
    \caption{Component-level optimization vs. phase-level AO across AP-user distance ($N = 40$). GWO component-level outperforms PSO component-level across all distances.}
...
\begin{figure}[htbp]
    ...
    \caption{Component-level optimization vs. phase-level AO across IRS size ($d = 498$ m). The performance gap between component-level and phase-level increases with $N$ due to the growing search space dimensionality ($4N$).}
    \label{fig:component_vs_N}
\end{figure}

Key observations:
\begin{enumerate}
    \item \textbf{GWO superiority at component level:} GWO component-level consistently outperforms PSO component-level across all distances and IRS sizes. This is attributed to GWO's three-leader guidance mechanism ($\alpha$, $\beta$, $\delta$), which improves exploration in the large, irregular $4N$-dimensional hardware search space and reduces premature convergence.

    \item \textbf{Phase-level advantage:} Phase-level AO consistently outperforms all component-level optimizers, as it operates in the lower-dimensional $N$-space with a well-characterized objective landscape. The performance gap grows with $N$ since the component-level search space scales as $4N$.

    \item \textbf{PSO vs. GWO at component level:} PSO follows a single global-best direction, making it prone to premature convergence in the 160-dimensional space ($4 \times 40$). GWO's use of three leaders provides better exploration-exploitation balance.
\end{enumerate}
...
\begin{enumerate}
    \item GWO converges faster and to a higher value than PSO component-level, benefiting from the quartic decay schedule that concentrates exploitation in the second half of iterations.
    \item PSO converges more slowly, as the single global-best direction provides less guidance in high-dimensional spaces.
\end{enumerate}
...
Key findings:
\begin{enumerate}
    \item \textbf{Fixing $R$ only:} Causes minimal performance loss (within $\sim$0.05 bits/s/Hz), suggesting that the series resistance has a secondary effect on the achievable rate.

    \item \textbf{Fixing $C$:} Causes large performance degradation, as the varactor capacitance $C$ is the primary parameter controlling the resonance frequency and phase response of the circuit. This is consistent with the physics of varactor-based IRS elements.

    \item \textbf{Fixing $C$ and $R$:} Further reduces performance, leaving only $L_1$ and $L_2$ as optimization variables, which limits the achievable phase and amplitude range.

    \item \textbf{Conclusion:} The capacitance $C$ is the most critical component for IRS optimization, as it directly controls the varactor's impedance and thus the reflection coefficient's phase and amplitude.
\end{enumerate}
...
Key observations:
\begin{enumerate}
    \item \textbf{Hybrid superiority:} All four hybrid variants outperform their standalone component-level counterparts, validating the warm-start strategy. The phase-level solution provides an excellent initialization that guides the component-level search toward the promising region of the parameter space.

    \item \textbf{Best hybrid variants:} The AO-based hybrids (Hybrid AO+PSO and Hybrid AO+GWO) slightly outperform the PSO-based hybrids, as AO provides a more reliable and faster phase-level solution.

    \item \textbf{Bridging the gap:} The hybrid approach significantly narrows the gap between phase-level and component-level optimization, achieving performance close to the phase-level AO baseline while operating directly on hardware parameters.
\end{enumerate}
...
\begin{table}[htbp]
    \centering
    \caption{Summary of achievable rates (bits/s/Hz) at $d = 498$ m.}
    \label{tab:summary}
    \begin{tabular}{l c c}
        \toprule
        \textbf{Scheme} & \textbf{$N = 40$} & \textbf{Level} \\
        \midrule
        Upper bound (ideal) & 3.30 & Phase \\
        AO practical (Prop. 1) & 2.32 & Phase \\
        AO practical (1D search) & 2.33 & Phase \\
        Ideal design, practical eval & 2.00 & Phase \\
        PSO (practical) & 2.32 & Phase \\
        GWO (practical) & --- & Phase \\
        \midrule
        PSO component & --- & Component \\
        GWO component & --- & Component \\
        Hybrid AO+PSO & --- & Hybrid \\
        Hybrid AO+GWO & --- & Hybrid \\
        \midrule
...
\begin{enumerate}
    ...
    \item \textbf{Metaheuristic competitiveness:} Population-based metaheuristics (PSO, GWO) can match the performance of the problem-specific AO algorithm for phase-level optimization, while offering greater flexibility and generality. These algorithms do not require problem-specific decomposition and can handle arbitrary objective functions.

    \item \textbf{GWO advantage at component level:} At the component level, GWO's three-leader guidance mechanism provides superior exploration-exploitation balance in the high-dimensional ($4N$) hardware parameter space, consistently outperforming PSO. The quartic decay schedule further improves convergence behavior.

    \item \textbf{Hybrid warm-start effectiveness:} The proposed hybrid approach, which uses the phase-level solution to warm-start the component-level optimizer, significantly improves performance over standalone component-level optimization. This bridges the gap between the fast but abstract phase-level optimization and the realistic but challenging component-level optimization.

    \item \textbf{Component sensitivity:} The ablation study reveals that the varactor capacitance $C$ is the most critical circuit parameter for IRS performance, as it directly controls the resonance frequency and phase response. Series resistance $R$ has a minimal impact, suggesting it can be fixed at a nominal value to reduce the search space dimensionality.
\end{enumerate}
